...
export default function Page() {
  ...
  useEffect(() => {
    load();
    const t = setInterval(load, 60_000); // 1분마다 화면 갱신
  ...
  return (
    <main style={{ padding: 20, fontFamily: "sans-serif" }}>
      ...
      <div style={{ fontSize: 12, opacity: 0.75 }}>
        마지막 갱신: {data.fetched_at ? new Date(data.fetched_at).toLocaleString() : "-"} /{" "}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginTop: 12 }}>
        {pick.map((p, i) => {
          ...
          return (
            <div key={i} style={{ border: "1px solid #ccc", padding: 12, borderRadius: 10 }}>
              <div style={{ fontWeight: 800, marginBottom: 8 }}>슬롯 {i + 1}</div>
